...
/** Каталог рабочих данных. В контейнере это том `/data`, локально — что задали. */
const DATA_DIR = path.resolve(process.env.SUB_LAB_DATA_DIR || process.env.SUB_MIRROR_DATA_DIR || "/data");
...
const SYNC_API_TOKEN = String(process.env.SYNC_API_TOKEN || "").trim();
...
const PROFILE_ROOT_DIRS = PROFILE_DIR_ENV
  ? [path.join(DATA_DIR, "profiles"), PROFILE_DIR_ENV]
  : [path.join(DATA_DIR, "profiles"), PROFILE_FALLBACK_DIR];
...
const OUT_RAW = path.join(DATA_DIR, "raw.txt");
const OUT_YAML = path.join(DATA_DIR, "subscription.yaml");
const OUT_STATUS = path.join(DATA_DIR, "status.json");
const OUT_CONVERTED = path.join(DATA_DIR, "converted.txt");
const SOURCE_PATH = path.join(DATA_DIR, "source.txt");
